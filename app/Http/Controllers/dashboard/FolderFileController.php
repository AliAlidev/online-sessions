...
class FolderFileController extends Controller
{
    ...
    function index(Request $request, $eventSlug, $folderSlug)
    {
        try {
            ...
            if ($request->ajax()) {
                $files = $folder->files()->when($folder->folder_type == 'video', function ($q) {
                    $q->orderBy('file_order', 'asc');
                })->when($folder->folder_type == 'image', function ($q) {
                ...
                return DataTables::of($files)
                    ...
                    ->editColumn('file_order', function ($row) {
                        return '<div class="folder-order-div">' . $row->file_order . '</div>';
                    })
                    ...
                    ->rawColumns(['file', 'status', 'name_and_size', 'file_order', 'actions'])
                    ->make(true);
            }
