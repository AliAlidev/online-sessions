        ...
        <div class="main-body">
            <div class="main-body-upload">
                <div class="upload-container">
                    ...
                    <a class="back-button"
                        href="{{ route('landing.gallery', ['year' => $year, 'month' => $month, 'customer' => $customer]) }}">
...
@push('scripts')
    ...
    <script>
        $('#uploadForm').submit(function(e) {
            ...
            uploadFile(file, 0, submitBtn).then((response) => {
                hideButtonLoader(submitBtn);
                $('#uploadForm')[0].reset();
        ...
        async function uploadFile(file, index, submitBtn) {
            return new Promise((resolve, reject) => {
                ...
                $.ajax({
                    url: "{{ route('landing.share-event-image') }}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(response) {
                        clearInterval($("#status-0").data("interval")); // Stop animated dots
                        $("#progress-bar-0").removeClass("bg-success").addClass("bg-primary").text(
                            "Completed");
                        $("#status-0").addClass('hidden-force');
                        var element = `<div class="alert alert-success"> ${response.message}</div>`;
                        $('#uploadForm').find('.modal-body').prepend(element);
                        hideButtonLoader(submitBtn);
                        resolve(); // Resolve the Promise after successful upload
                    },
                    error: function(jqXHR, textStatus) {
                        clearInterval($("#status-0").data(
                            "interval")); // Stop animated dots
                        $("#progress-bar-0").removeClass("bg-success").addClass(
                            "bg-danger").text("Failed");

                        let errorMessage = (textStatus === "timeout") ?
                            "Stage2: Upload timed out" : "Stage2: Failed";
                        $("#status-0").text(errorMessage).show();

                        showRetryButton(0); // Show retry button only when failed
